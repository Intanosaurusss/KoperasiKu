...
<div class="bg-white pt-1 px-2 pb-2 rounded-md shadow-sm">
    ...
    <div class="flex flex-col md:flex-row items-center mt-4 w-full space-y-3">
        <div class="flex order-1 md:order-1 w-full max-w-full">
            <form method="GET" action="{{ route('riwayat-petugas.index') }}" id="search-form" class="w-full md:w-auto space-x-2">
        ...
        <form  action="{{ route('cetakriwayatpetugasbydate') }}" method="POST" class="flex flex-wrap justify-end md:flex-nowrap items-center gap-2 md:pl-2 ml-auto order-2 md:order-1 w-full max-w-full">
            @csrf
            ...
            @foreach ($transaksi as $item)
                <tr class="hover:bg-gray-100">
                    ...
                    <td class="flex px-6 py-2 whitespace-nowrap text-sm text-gray-900 space-x-2 md:space-x-6 justify-center">
                        @include('components.crud-riwayat-petugas', ['item' => $item])
                    </td>
                </tr>
            @endforeach
            @endif
...
<script>
    ...
    detailButtons.forEach(button => {
        button.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`/riwayat-petugas/${transaksiId}`);
