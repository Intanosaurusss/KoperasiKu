...
<div class="bg-white pt-1 px-2 pb-2 rounded-md shadow-sm">
    ...
    <div class="flex flex-col md:flex-row items-center mt-4 w-full space-y-3 max-w-full">
        ...
        <form action="{{ route('cetakpengeluaranbydate.cetak') }}" method="GET" class="flex flex-wrap justify-end md:flex-nowrap items-center gap-2 md:pl-2 ml-auto order-2 md:order-1 w-full max-w-full">
        @csrf
        <div class="flex items-center gap-0.5 w-full md:w-auto max-w-full">
            <input type="date" name="date_start" class="border border-gray-300 rounded-lg p-1.5 text-xs @error('date_start') border-red-500 @else border-gray-300 @enderror" placeholder="Tanggal Awal" max="{{ now()->toDateString() }}" />
            <span class="mx-1 text-xs">s.d.</span>
            <input type="date" name="date_end" class="border border-gray-300 rounded-lg p-1.5 text-xs @error('date_end') border-red-500 @else border-gray-300 @enderror" placeholder="Tanggal Akhir" max="{{ now()->toDateString() }}" />
        </div>
        
        <button type="submit" class="bg-blue-400 hover:bg-blue-500 p-1 rounded-md flex items-center text-white justify-center md:w-auto mt-2 md:mt-0">
