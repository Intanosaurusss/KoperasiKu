...
<header class="flex flex-wrap fixed md:justify-start md:flex-nowrap z-50 w-full py-2 bg-white shadow-md">
  <nav class="relative max-w-7xl w-full flex flex-wrap md:grid md:grid-cols-12 basis-full items-center px-4 md:px-6 mx-auto">
    <div class="md:col-span-3 flex items-center gap-2">
      <!-- Logo -->
       <div class="">
        <img src="{{ asset('assets/logo_koperasiku.png') }}" alt="image description" class="rounded-full bg-white w-12 h-12">
       </div>
      <!-- End Logo -->
    ...
    <a href="login" type="button" class="py-2 px-4 md:border-2 md:hover:bg-purple-400 border border-purple-400 md:border-purple-400 inline-flex items-center gap-x-2 text-sm font-medium rounded-xl md:rounded-full focus:outline-none transition ease-in-out duration-300">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-purple-400 block md:hidden w-5 h-5">
    ...
    <div id="hs-navbar-hcail" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow md:block md:w-auto md:basis-auto md:order-2 md:col-span-6 bg-navbar-default md:bg-transparent rounded-lg pl-4 pb-4 pt-2 z-50 mt-2" aria-labelledby="hs-navbar-hcail-collapse">
      <div class="flex flex-col gap-y-4 gap-x-0 mt-5 md:flex-row md:justify-center md:items-center md:gap-y-0 md:gap-x-7 md:mt-0">
        <div>
          <a href="#beranda" class="relative inline-block hover:text-purple-400 focus:outline-none text-gray-700" href="#" aria-current="page" id="link-beranda">Beranda</a>
        </div>
        <div>
          <a href="#tentang" class="inline-block hover:text-purple-400 focus:outline-none text-gray-700" id="link-tentang">Tentang</a>
        </div>
        <div>
          <a href="#produk" class="inline-block hover:text-purple-400 focus:outline-none text-gray-700"id="link-produk">Produk</a>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const sections = ['beranda', 'tentang', 'produk'];
  ...
  menuToggle.addEventListener('click', function () {
    if (window.innerWidth < 768) {
      if (menu.classList.contains('hidden')) {
        ...
        menu.classList.add('bg-purple-100');
      } else {
  ...
  window.addEventListener('resize', function () {
    if (window.innerWidth >= 768) {
      ...
      menu.classList.remove('hidden');
      menu.classList.remove('bg-purple-100');
    ...
    sections.forEach((section, index) => {
      ...
      if (sectionElement) {
        ...
        if (scrollPos >= sectionTop && scrollPos < sectionTop + sectionHeight) {
          links.forEach(link => link.classList.remove('border-b-2', 'border-purple-400'));
          links[index].classList.add('border-b-2', 'border-purple-400');
        }
      }
    });
  }
  ...
  links.forEach(link => {
    link.addEventListener('click', function () {
      links.forEach(l => l.classList.remove('border-b-2', 'border-purple-400'));
      this.classList.add('border-b-2', 'border-purple-400');
    });
  });
});
</script>
