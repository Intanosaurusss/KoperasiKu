<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>About</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@100;200;300;400;500;600;700;800;900&display=swap" rel="stylesheet">
    <style>
        body {
            font-family: 'Poppins', sans-serif;
        }
    </style>
</head>
<body>
<section id="tentang" class="bg-white" data-aos="zoom-in">
<div class="text-center pt-10 space-y-3">
        <h2 class="text-2xl md:text-4xl tracking-tight font-bold text-purple-400 inline-block relative">
            Tentang Kami
            <span class="absolute left-1/2 transform -translate-x-1/2 -bottom-3 bg-purple-400 h-0.5 w-12"></span>
        </h2>
        </div>

    <!-- div untuk kedua kontainer -->
    <div class="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6 text-gray-700">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 h-full">

            <!-- Kontainer Kiri: Teks dan List -->
            <div class="flex flex-col justify-between">
                <div>
                    <p class="mb-4">IDNeobilling adalah Applikasi Manajemen untuk pengusaha ISP/mini ISP dalam mengatur manajemen pelanggan.</p>
                    <ul class="space-y-2 text-left text-gray-500 dark:text-gray-700">
                        <li class="flex items-center space-x-3 rtl:space-x-reverse">
                            <svg class="flex-shrink-0 w-3.5 h-3.5 text-purple-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5"/>
                            </svg>
                            <span>Efisiensi Waktu</span>
                        </li>
                        <li class="flex items-center space-x-3 rtl:space-x-reverse">
                            <svg class="flex-shrink-0 w-3.5 h-3.5 text-purple-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5"/>
                            </svg>
                            <span>Tidak perlu antri panjang</span>
                        </li>
                        <li class="flex items-center space-x-3 rtl:space-x-reverse">
                            <svg class="flex-shrink-0 w-3.5 h-3.5 text-purple-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 12">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5.917 5.724 10.5 15 1.5"/>
                            </svg>
                            <span>Dapat membayar cashless</span></span>
                        </li>
                    </ul>
                </div>
            </div>

            <!-- Kontainer Kanan: Teks dan Button -->
            <div class="flex flex-col">
                <div>
                    <p>Kantinkita ini <span class="font-bold text-purple-400"> diharapkan dapat memberikan kemudahan bagi siswa untuk membeli makanan</span></p>
                    <p class="mb-6">Dengan fasilitas dan kemudahan layanan yang kami berikan, tidak perlu lagi deh antri panjang saat membeli makan di kantin!.</p>
                </div>
                <div>
                    <a href="login" type="button" class="py-2 px-5 text-sm font-medium text-purple-400 hover:text-white focus:outline-none bg-white rounded-lg border-2 border-purple-400 focus:z-10 hover:bg-purple-400">
                        Mulai sekarang
                    </a>
                </div>
            </div>
            
    </div>
</div>
</section>

</body>
</html>
